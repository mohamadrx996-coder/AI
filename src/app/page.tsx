...
import { Send, Bot, User, Copy, Check, Trash2, Terminal, Loader2, Zap, Key, Clock, Shield, MessageCircle, ExternalLink, X, Eye, EyeOff, Plus, Trash, Gift, CheckCircle2, AlertCircle } from 'lucide-react';
...
// ─── مودال عرض الفقراء ──────────────────────────────────────────────
function FreeKeyModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: (key: string) => void }) {
  ...
  const handleSubmit = async () => {
    if (!groqKey.trim()) return setError('أدخل مفتاح Groq أولاً');
    if (!groqKey.trim().startsWith('gsk_')) return setError('المفتاح يجب أن يبدأ بـ gsk_');
    setStep('loading');
    setError('');
    try {
      ...
      if (!res.ok) { setStep('error'); setError(data.error || 'فشل التحقق'); return; }
      setServiceKey(data.serviceKey);
      setStep('success');
    } catch { setStep('error'); setError('خطأ في الاتصال'); }
  };
  ...
  const useKey = () => { onSuccess(serviceKey); onClose(); };
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg">700 حرف مجاني</span>
          </div>
        ...
        <div className="p-5 space-y-4">
          {step === 'form' && (
            <>
              ...
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground font-medium">اتبع هذه الخطوات:</p>
                {[
                  { n: '1', text: 'اذهب لـ', link: 'https://console.groq.com', label: 'console.groq.com' },
                  { n: '2', text: 'سجّل حساب مجاني (بدون بطاقة)', link: null, label: null },
                  { n: '3', text: 'API Keys → Create API Key', link: null, label: null },
                  { n: '4', text: 'انسخ المفتاح وضعه أدناه', link: null, label: null },
                ].map((s) => (
                  <div key={s.n} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/40">
                    <span className="w-6 h-6 rounded-full bg-purple-500/20 text-purple-400 text-xs font-bold flex items-center justify-center shrink-0">{s.n}</span>
                    <span className="text-sm text-foreground">
                      {s.text}{' '}
                      {s.link && <a href={s.link} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline inline-flex items-center gap-1">{s.label} <ExternalLink className="w-3 h-3" /></a>}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">مفتاح Groq الخاص بك</label>
              ...
              <Button onClick={handleSubmit} className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 hover:opacity-90">
                <Gift className="w-4 h-4 mr-2" />
                احصل على 700 حرف مجاني
              </Button>

              <p className="text-xs text-center text-muted-foreground">
                مفتاحك لن يُستخدم في الموقع — فقط للتحقق من صحته
              </p>
            </>
          )}
          ...
          {step === 'loading' && (
            <div className="py-8 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground">جارٍ التحقق من المفتاح...</p>
            </div>
          )}
          ...
          {step === 'success' && (
            <div className="space-y-4">
              <div className="text-center space-y-2">
                ...
                <p className="font-bold text-lg">تم التحقق! 🎉</p>
                <p className="text-sm text-muted-foreground">هذا كودك بـ 700 حرف مجاني</p>
              </div>
              <div className="bg-muted rounded-xl p-4 space-y-2">
                <p className="text-xs text-muted-foreground text-center">كود الخدمة</p>
                <div className="flex items-center gap-2">
                  <code className="flex-1 text-center font-mono text-lg font-bold tracking-widest text-emerald-400">{serviceKey}</code>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button variant="outline" onClick={copyKey} className="text-sm">
                  {copied ? '✓ تم النسخ' : 'نسخ الكود'}
                </Button>
                <Button onClick={useKey} className="text-sm bg-gradient-to-r from-purple-500 to-pink-500 hover:opacity-90">
                  استخدمه الآن
                </Button>
              </div>
            </div>
          ...
          {step === 'error' && (
            <div className="space-y-4">
              ...
              <Button onClick={() => { setStep('form'); setError(''); }} variant="outline" className="w-full">
                حاول مرة أخرى
              </Button>
            </div>
          )}
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-lg mb-5">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <Button
              ...
              className="text-xs border-emerald-500/40 text-emerald-500 hover:bg-emerald-500/10 gap-1.5"
            >
